...
export const PlanCreatePageWrapper: React.FC<{
  namespace: string;
}> = ({ namespace }) => {
  //   console.log(ALL);
    ...
    <>
      <NamespaceBar>
        {/* {activeNamespace === '#ALL_NS#' && (
          <Alert variant="warning" isInline title={'No namespace selected'} />
        )} */}
      </NamespaceBar>
      {activeNamespace === '#ALL_NS#' && (
        <Alert className="co-alert" isInline variant="warning" title={t('Select a namespace')}>
          <ForkliftTrans>
            This plan will be created in the <strong>{defaultNamespace}</strong> namespace, if you
            wish to choose another namespace, choose a namespace from the top bar.
          </ForkliftTrans>
        </Alert>
      )}
      <PlanCreatePage namespace={activeNamespace === '#ALL_NS#' ? '' : activeNamespace} />
    </>
  );
};

export default PlanCreatePageWrapper;
