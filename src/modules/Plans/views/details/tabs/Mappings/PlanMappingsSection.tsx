...
export const PlanMappingsSection: React.FC<PlanMappingsSectionProps> = ({
  ...
  const ovirtFindObj = (obj, nextName: string) => obj.path === nextName || obj.name === nextName;

  const vsphereFindObj = (obj, nextName: string) => obj.path === nextName || obj.name === nextName;

  const openstackFindObj = (obj, nextName: string) =>
    obj.path === nextName || obj.name === nextName;

  const openshiftFindObj = (
    obj: OpenShiftNetworkAttachmentDefinition | OpenShiftStorageClass,
    nextName: string,
  ) => `${obj.namespace}/${obj.name}` === nextName || obj.name === nextName;

  const ovaFindObj = (obj, nextName: string) => obj.path === nextName || obj.name === nextName;
